...
if ([string]::IsNullOrWhiteSpace($SweepId)) {
    $SweepId = "gridoutlier_fixedstep_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}
...
$experiments = @(
    @{ Suffix = "o00010"; Outlier = "0.00010" },
    @{ Suffix = "o00025"; Outlier = "0.00025" }
)

foreach ($experiment in $experiments) {
    $runId = "{0}_{1}" -f $SweepId, $experiment.Suffix
    $runLog = Join-Path $root "logs\$runId.txt"
    Add-Content -Path $controllerLog -Value ("[{0}] START {1} outlier={2}" -f (Get-Date -Format s), $runId, $experiment.Outlier)
    $args = @(
        ...
        "-TernaryRegWeight", "0",
        "-OutlierRegWeight", $experiment.Outlier,
