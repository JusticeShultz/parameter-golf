...
if ([string]::IsNullOrWhiteSpace($SweepId)) {
    $SweepId = "repeatrtsweep_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}
...
function Test-RoundtripRunComplete {
    ...
    $finalExact = Select-String -Path $LogPath -Pattern '^final_int8_zlib_roundtrip_exact ' -Quiet
    if ($finalExact) {
        return $true
    }
...
$experiments = @(
    ...
    @{ Suffix = "base_c";   CacheMix = "0.0";  BigramMix = "0.0";  CacheSize = "0" },
    @{ Suffix = "side_c";   CacheMix = "0.02"; BigramMix = "0.03"; CacheSize = "8" }
)
