...
if ([string]::IsNullOrWhiteSpace($SweepId)) {
    $SweepId = "exportaware_fixedstep_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}
...
$experiments = @(
    @{ Suffix = "g010_r000"; Grid = "0.10"; Rank1 = "0.0" },
    @{ Suffix = "g025_r000"; Grid = "0.25"; Rank1 = "0.0" },
    @{ Suffix = "g010_r050"; Grid = "0.10"; Rank1 = "0.5" },
    @{ Suffix = "g025_r050"; Grid = "0.25"; Rank1 = "0.5" }
)

foreach ($experiment in $experiments) {
    $runId = "{0}_{1}" -f $SweepId, $experiment.Suffix
    $runLog = Join-Path $root "logs\$runId.txt"
    Add-Content -Path $controllerLog -Value ("[{0}] START {1} grid={2} rank1={3}" -f (Get-Date -Format s), $runId, $experiment.Grid, $experiment.Rank1)
    $args = @(
        ...
        "-CompressionGridRegWeight", $experiment.Grid,
        "-CompressionRank1RegWeight", $experiment.Rank1,
        "-TernaryRegWeight", "0",
        "-OutlierRegWeight", "0",
