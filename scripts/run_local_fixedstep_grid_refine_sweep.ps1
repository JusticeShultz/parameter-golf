...
if ([string]::IsNullOrWhiteSpace($SweepId)) {
    $SweepId = "gridrefine_fixedstep_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}
...
$experiments = @(
    @{ Suffix = "g0080"; Grid = "0.08" },
    @{ Suffix = "g0120"; Grid = "0.12" }
)

foreach ($experiment in $experiments) {
    $runId = "{0}_{1}" -f $SweepId, $experiment.Suffix
    $runLog = Join-Path $root "logs\$runId.txt"
    Add-Content -Path $controllerLog -Value ("[{0}] START {1} grid={2}" -f (Get-Date -Format s), $runId, $experiment.Grid)
    $args = @(
