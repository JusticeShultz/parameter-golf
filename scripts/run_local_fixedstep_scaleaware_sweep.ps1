...
if ([string]::IsNullOrWhiteSpace($SweepId)) {
    $SweepId = "scaleaware_fixedstep_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}
...
$experiments = @(
    @{ Suffix = "g010_s0010"; Grid = "0.10"; Scale = "0.0010" },
    @{ Suffix = "g010_s0025"; Grid = "0.10"; Scale = "0.0025" }
)

foreach ($experiment in $experiments) {
    $runId = "{0}_{1}" -f $SweepId, $experiment.Suffix
    $runLog = Join-Path $root "logs\$runId.txt"
    Add-Content -Path $controllerLog -Value ("[{0}] START {1} grid={2} scale={3}" -f (Get-Date -Format s), $runId, $experiment.Grid, $experiment.Scale)
    $args = @(
        ...
        "-CompressionScaleRegWeight", $experiment.Scale,
